...
namespace :import do
  ...
  task :whitehall_migration, %i[organisation_slug document_type document_subtypes] => :environment do |_, args|
    ...
    puts "#{Plek.new.external_url_for("content-publisher")}/whitehall-migration/#{whitehall_migration.id}"
  end
  ...
  task :whitehall_document, [:document_id] => :environment do |_, args|
    ...
    puts "#{Plek.new.external_url_for("content-publisher")}/whitehall-migration/#{whitehall_import.id}"
  end
end
