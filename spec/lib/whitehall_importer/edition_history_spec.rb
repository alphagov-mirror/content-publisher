...
RSpec.describe WhitehallImporter::EditionHistory do
  ...
  describe "#timeline_entry" do
    it "returns first_created for create event on first edition" do
      ...
      expect(instance.timeline_entry(event, 1)).to eq("first_created")
    end
    ...
    it "returns new_edition for create event on subsequent edition" do
      ...
      expect(instance.timeline_entry(event, 2)).to eq("new_edition")
    end
    ...
    it "returns document_updated if there is no change in state" do
      ...
      expect(instance.timeline_entry(second_event, 1)).to eq("document_updated")
    end

    it "returns the Content Publisher event key if the state has changed" do
      ...
      expect(instance.timeline_entry(second_event, 1)).to eq("published")
    end
    ...
    it "aborts if the state has changed but the mapping is undefined" do
      ...
      expect { instance.timeline_entry(second_event, 1) }
        .to raise_error(WhitehallImporter::AbortImportError)
    end
  end
end
